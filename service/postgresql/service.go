...
// Postgresql provides the API operation methods for making requests to
// postgresql. See this package's package overview docs
// for details on the service.
//
// Postgresql methods are safe to use concurrently. It is not safe to
...
const (
	ServiceName = "postgresql" // Name of service.
	EndpointsID = ServiceName  // ID to lookup a service endpoint with.
	ServiceID   = "postgresql" // ServiceID is a unique identifer of a specific service.
)

// New creates a new instance of the Postgresql client with a session.
// If additional configuration is needed for the client instance use the optional
// ksc.Config parameter to add your extra config.
//
// Example:
//     // Create a Postgresql client from just a session.
//     svc := postgresql.New(mySession)
//
//     // Create a Postgresql client with additional configuration
//     svc := postgresql.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
func New(p client.ConfigProvider, cfgs ...*aws.Config) *Postgresql {
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Postgresql {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func SdkNew(p client.ConfigProvider, cfgs *ksc.Config, info ...*utils.UrlInfo) *Postgresql {
	_info := utils.UrlInfo{}
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Postgresql {
	svc := &Postgresql{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Postgresql operation and runs any
// custom request initialization.
func (c *Postgresql) newRequest(op *request.Operation, params, data interface{}) *request.Request {
